...
Move `instagrapi-rest` from synchronous `instagrapi` to asynchronous `aiograpi` from PyPI, update the project to Python 3.13, cover the local REST adapter with tests, add `GET /user/about`, align HTTP methods with REST semantics, publish API version `3.1.1`, and verify the final diff with Claude.

## Fixed Decisions

- Runtime target: Python 3.13.
- API version: `3.1.1` because the REST method and path cleanup was a breaking public API change in `3.0.0`, followed by client-friendly OpenAPI schema naming in `3.1.0` and tag display cleanup in `3.1.1`.
...
- Verify OpenAPI generation includes all routers, `GET /user/about`, API version `3.1.1`, slash-separated paths, client-friendly schemas, human-friendly tags, and the REST HTTP method map.
